  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        sessionStorage.removeItem('accessToken'); localStorage.removeItem('accessToken'); localStorage.removeItem('auth-storage');
...
export const chatService = {
  ...
  getOrCreateConversation: (participantId) => api.post(`/chat/conversations/${participantId}/get-or-create`),
